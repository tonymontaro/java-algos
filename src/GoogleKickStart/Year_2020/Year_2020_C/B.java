...
//    private static final FastReader in = new FastReader();
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 1; t <= cases; t++) {
            int rowLen = in.intNext(), colLen = in.intNext();
            int[][] grid = new int[rowLen][colLen];
            for (int r = 0; r < rowLen; r++) {
                String row = in.next();
                for (int c = 0; c < colLen; c++) {
                    grid[r][c] = row.charAt(c) - 'A';
                }
            }

            HashSet<Integer> chars = new HashSet<>();
            int[] prev = new int[colLen];
            adj = new HashSet[27];
            ...
            for (int r = rowLen-1; r >= 0; r--) {
                for (int c = 0; c < colLen; c++) {
                    int ch = grid[r][c];
                    chars.add(ch);
                    if (r < rowLen-1 && prev[c] != ch) {
                        adj[ch].add(prev[c]);
                    }
                    prev[c] = ch;
                }
            }
            out.println(String.format("Case #%d: %s", t, topologicalSort(chars)));
        }
    ...
    static String topologicalSort(HashSet<Integer> items) {
        invalid = false;
        boolean[] visited = new boolean[27];
        boolean[] visiting = new boolean[27];
        StringBuilder sorted = new StringBuilder();
        for (int i: items) {
            if (!visited[i]) topologicalSortHelper(i, sorted, visited, visiting);
        }
        if (invalid) return "-1";
        return sorted.toString();
    }

    static void topologicalSortHelper(int node, StringBuilder sorted, boolean[] visited, boolean[] visiting) {
        ...
        for (int child : adj[node]) {
            ...
            topologicalSortHelper(child, sorted, visited, visiting);
        }
        visiting[node] = false;
        sorted.append((char)(node + 'A'));
    }
