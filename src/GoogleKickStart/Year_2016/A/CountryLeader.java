...
//    private static final FastReader in = new FastReader();
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 1; t <= cases; t++) {
            ...
            for (int i = 0; i < n; i++) {
                arr[i] = in.line().trim();
                counts.put(arr[i], charCount(arr[i]));
            }

            java.util.Arrays.sort(arr, new Comparator<String>() {
                public int compare(String a, String b) {
                    if (counts.get(a).equals(counts.get(b))) return a.compareTo(b);
                    return Integer.compare(counts.get(b), counts.get(a));
                }
            });

            out.println(String.format("Case #%d: %s", t, arr[0]));
        }
