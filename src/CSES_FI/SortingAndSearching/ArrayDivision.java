...
public class ArrayDivision {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int n = in.intNext(), maxDiv = in.intNext();
    ...
    static boolean canDivide(long maxTotal, int maxDiv, long[] arr) {
        int count = 1;
        long total = 0;
        for (long num : arr) {
            if (total + num > maxTotal) {
                total = 0;
                count++;
            }
            total += num;

            if (num > maxTotal || count > maxDiv) return false;
        }
        return true;
    }
