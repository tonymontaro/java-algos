...
public class Towers {
    ...
    public static void main(String[] args) throws IOException {
        ...
        out.println(longestIncreasingSubArray(n, arr));

        out.close();
    }

    static int longestIncreasingSubArray(int n, long[] arr) {
        ArrayList<Long> binaryArr = new ArrayList<>();
        Collections.fill(binaryArr, Long.MAX_VALUE);
        binaryArr.add(arr[0]);
        for (int i = 1; i < n; i++) {
            int low = ArrUtil.upperBound(binaryArr, arr[i]);
            if (low >= binaryArr.size()) binaryArr.add(arr[i]);
            else if (arr[i] < binaryArr.get(low)) binaryArr.set(low, arr[i]);
        }
