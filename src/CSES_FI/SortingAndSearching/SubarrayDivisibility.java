...
public class SubarrayDivisibility {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int n = in.intNext();
        HashMap<Integer, Integer> prefixSumModCounts = new HashMap<>();
        prefixSumModCounts.put(0, 1);
        long count = 0;
        int total = 0;
        for (int i = 0; i < n; i++) {
            total = (total + (in.intNext() % n) + n) % n;
            if (prefixSumModCounts.containsKey(total)) count += prefixSumModCounts.get(total);
            prefixSumModCounts.merge(total, 1, Integer::sum);
        }
        out.println(count);
