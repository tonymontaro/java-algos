...
public class MaximumSubarraySumII {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) prefixSum[i + 1] = arr[i] + prefixSum[i];

        SortedSet<Tuple> validPrefixes = new TreeSet<>();
        long best = Long.MIN_VALUE;
        for (int i = 0; i <= n; i++) {
            if (i >= a) validPrefixes.add(new Tuple(prefixSum[i - a], i - a));

            if (validPrefixes.size() > 0) best = Math.max(best, prefixSum[i] - validPrefixes.first().a);

            if (i >= b) validPrefixes.remove(new Tuple(prefixSum[i - b], i - b));
        }
        out.println(best);
