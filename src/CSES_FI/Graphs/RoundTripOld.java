...
public class RoundTripOld {
    ...
    static boolean cycleComplete = false;
    static String solve(int n) {
        boolean[] visited = new boolean[n + 1];

        for (int i = 1; i < n + 1; i++) {
            if (!visited[i] && isCycle(i, 0, visited)) return count + "\n" + result.toString();
        }

        return "IMPOSSIBLE";
    }

    static boolean isCycle(int node, int parent, boolean[] visited) {
        if (visited[node]) {
            count++;
            result.append(node);
            startNode = node;
        ...
        for (int child : adj[node]) {
            if (child != parent && child != node && isCycle(child, node, visited)) {
                if (!cycleComplete) {
                    result.append(" ").append(node);
                    count++;
                    if (node == startNode) cycleComplete = true;
                }
                return true;
            }
        }
    ...
    static class CF_Reader {
        ...
        public HashSet<Integer>[] adjacencyList(int n, int m) throws IOException {
