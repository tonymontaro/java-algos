...
public class TopologicalSort {
    ...
    static ArrayList<Integer> topologicalSort(int n) {
        ...
        for (int i = 1; i < n + 1; i++) {
            if (!visited[i]) topologicalSortHelper(i, sorted, visited);
        }
        return sorted;
    }

    static void topologicalSortHelper(int node, ArrayList<Integer> sorted, boolean[] visited) {
        if (visited[node]) return;
        visited[node] = true;
        for (int child : adj[node]) {
            topologicalSortHelper(child, sorted, visited);
        }
        sorted.add(node);
    }
