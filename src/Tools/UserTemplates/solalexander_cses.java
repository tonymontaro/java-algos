...
public class solalexander_cses {
    ...
    private static void solve() {
        ...
        // find median of first interval
        for (int i = 0; i < K; i++) heap.add(new Pair(a[i], i));

        Iterator<Pair> it = heap.iterator();
        for (int i = 0; i < LK-1; i++) it.next();
        Pair median = it.next();

        writer.print(median.v1).print(" ");

        // calculate running median
        for (int i = K; i < N; i++) {

            Pair retiree = new Pair(a[i - K], i-K);
            Pair candidate = new Pair(a[i], i);

//      System.out.println(median + " " + retiree + " " + candidate + " " + heap);

            heap.remove(retiree);
            heap.add(candidate);

            if (retiree.compareTo(median) == -1 && candidate.compareTo(median) == 1) median = heap.higher(median);
            if (retiree.compareTo(median) == 1 && candidate.compareTo(median) == -1) median = heap.lower(median);

            if (retiree.compareTo(median) == 0 && candidate.compareTo(median) == -1) median = heap.lower(median);
            if (retiree.compareTo(median) == 0 && candidate.compareTo(median) == 1) median = heap.higher(median);
