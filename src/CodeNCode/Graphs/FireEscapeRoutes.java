...
// https://www.codechef.com/MARCH13/problems/FIRESC
public class FireEscapeRoutes {
    ...
    static String solve(int n) {
        ...
        for (int i = 1; i < n + 1; i++) {
            if (!visited[i]) {
                counts += 1;
                res = (res * countConnected(i, visited)) % MOD;
            }
        }
        return counts + " " + res + "\n";
    }

    static int countConnected(int node, boolean[] visited) {
        if (visited[node]) return 0;
        int res = 1;
        visited[node] = true;
        for (int child : adj[node]) {
            res += countConnected(child, visited);
        }
        return res;
    }
