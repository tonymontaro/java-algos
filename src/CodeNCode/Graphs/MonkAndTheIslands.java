...
public class MonkAndTheIslands {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 1; t <= cases; t++) {
            ...
            out.println(shortestPathLen(n));
        }

        out.close();
    }

    static int shortestPathLen(int n) {
        int[] distances = new int[n + 1];
        boolean[] visited = new boolean[n + 1];
        Deque<Integer> processing = new LinkedList<>();
        processing.add(1);
        visited[1] = true;
        while (!processing.isEmpty()) {
            int node = processing.pollFirst();
            if (node == n) return distances[n];
