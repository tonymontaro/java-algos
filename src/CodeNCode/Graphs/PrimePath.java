...
public class PrimePath {
    ...
    static String solve(int prime1, int prime2) {
        HashMap<Integer, Integer> visited = new HashMap<>();
        Deque<Integer> deque = new LinkedList<>();
        deque.add(prime1);
        visited.put(prime1, 0);
        while (!deque.isEmpty()) {
            int node = deque.pollFirst();
            int distance = visited.get(node);
            if (node == prime2) return String.valueOf(distance);
            ArrayList<Integer> adjacentPrimes = getAdjacentPrimes(node);
            for (int child : adjacentPrimes) {
                if (!visited.containsKey(child)) {
                    visited.put(child, distance + 1);
                    deque.add(child);
                }
            }
        }
    ...
    static ArrayList<Integer> getAdjacentPrimes(int prime) {
        ArrayList<Integer> adjPrimes = new ArrayList<>();
        for (int i = 1; i <= 1000; i *= 10) {
            int p1 = prime / i;
            int p2 = prime % i;
            for (int j = 0; j < 10; j++) {
                if (i == 1000 && j == 0 || i == 1 && j % 2 == 0 || j == p1 % 10) continue;
                int first = (p1 - (p1%10)) + j;
                int total = first * i + p2;
                if (primes.contains(total)) adjPrimes.add(total);
            }
        }
        return adjPrimes;
    }

    static HashSet<Integer> generatePrimes(int n) {
        int[] lp = new int[n + 1];
