...
public class LongestPathInATree {
    ...
    static int[] best = new int[]{0, 1};

    static int solve(int n) {
        boolean[] visited = new boolean[n + 1];
        setFurthest(1, 0, visited);
        visited = new boolean[n + 1];
        setFurthest(best[1], 0, visited);
        return best[0];
    }

    static void setFurthest(int node, int distance, boolean[] visited) {
        if (visited[node]) return;
        visited[node] = true;
        if (distance > best[0]) best = new int[]{distance, node};
        for (int child : adj[node]) {
            setFurthest(child, distance + 1, visited);
        }
    }
