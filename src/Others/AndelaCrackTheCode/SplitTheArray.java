...
public class SplitTheArray {
    ...
    public static void main(String[] args) throws IOException {
        ...
        List<Integer> arr = Arrays.asList(3, 5, 7, 11, 2, 3, 7);
        out.println(splitTheArray(arr));
    ...
    public static int splitTheArray(List<Integer> arr) {
        int n = arr.size();
        pr = new ArrayList<>();
        primes = new HashSet<>();
        generatePrimes(1000000);
        HashMap<Integer, Integer> minSplits = new HashMap<>();
        int[] dp = new int[n + 1];
        for (int i = 1; i < n + 1; i++) {
            int num = arr.get(i - 1);
            ...
            for (int prime : pr) {
                if (primes.contains(num)) {
                    minSplits.put(num, Math.min(minSplits.getOrDefault(num, Integer.MAX_VALUE), dp[i - 1] + 1));
                    minVal = Math.min(minVal, minSplits.get(num));
                    break;
                }
                if (num % prime == 0) {
                    minSplits.put(prime, Math.min(minSplits.getOrDefault(prime, Integer.MAX_VALUE), dp[i - 1] + 1));
                    while (num % prime == 0) num /= prime;
                    minVal = Math.min(minVal, minSplits.get(prime));
                }
                if (num == 1) break;
            }
            dp[i] = minVal;
