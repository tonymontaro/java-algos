...
public class D {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 0; t < cases; t++) {
            int n = in.intNext(), moder = in.intNext();
            long[] nums = in.nextLongArray(n);

            result.append(solve(nums, moder)).append("\n");
        }
        out.println(result);
    ...
    static long solve(long[] nums, int moder) {
        HashMap<Long, Long> mods = new HashMap<>();
        long max = 0;

        for (long num : nums) {
            long mod = moder - (num % moder);
            if (mod == moder) continue;
            if (mods.containsKey(mod)) mods.put(mod, mods.get(mod) + moder);
            else mods.put(mod, mod);
            max = Math.max(mods.get(mod), max);
        }
